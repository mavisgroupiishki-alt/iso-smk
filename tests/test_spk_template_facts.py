...
def test_spk_si_copy_list_keeps_every_listed_tool_and_two_thermometers():
    source = '''
Перечень средств измерения:
- термометр -50 °С - +50 °С;
- влагомер;
...
'''
    ...
    assert [tool['name'] for tool in tools] == [
        'Термометр', 'Влагомер', 'Рулетка измерительная', 'Рейка контрольная',
        'Нивелир', 'Рейка нивелирная', 'Термометр', 'Линейка измерительная',
        'Адгезиметр', 'Теодолит', 'Уровень строительный',
        'Клин для контроля зазоров', 'Штангенциркуль ШЦ', 'Угольник поверочный',
        'Набор щупов', 'Манометр', 'Плотномер динамический',
        'Ключ динамометрический', 'Гигрометр', 'Толщиномер', 'Дефектоскоп',
    ]
