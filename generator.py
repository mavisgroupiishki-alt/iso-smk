...
def gen_spk_spravka_si(company, dates, resp, api_key):
    """СПК Документ 8: Справка СИ — по формату КастомИнвест/ЮПротех"""
    full = f"{company.get('form','ООО')} «{_clean(company)}»"
    dir_init = _initials(_fio(resp.get('director')))
    year = dates['year']
...
1 | Нивелир ATLAS KL24 | Класс точности (погрешность): 2,5 мм/км | 1 | 819260 | Свидетельство о поверке № 194Н от 14.03.{int(year)-1}
2 | Рейка нивелирная телескопическая | Диапазон измерений: (0-5000) мм | 1 | — | СЧЕТ № 524 от 30-01-{year}
3 | Линейка измерительная | Диапазон измерений: (0-1000) мм | 1 | — | СЧЕТ № 524 от 30-01-{year}
4 | Рулетка измерительная | Диапазон измерений: (0-3000) мм | 1 | — | СЧЕТ № 524 от 30-01-{year}
5 | Уровень строительный | ГОСТ 9416, I группа точности | 1 | — | Свидетельство о поверке
