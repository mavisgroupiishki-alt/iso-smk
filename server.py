...
# Сессия сохраняется на доверенном рабочем устройстве месяц; "Выйти" или
# отключение учётной записи у владельца немедленно прекращают доступ.
_AUTH_SESSION_SECONDS = 60 * 60 * 24 * 30
...
11. Если в сообщении есть блок «НАЙДЕНЫ РАНЕЕ СОХРАНЁННЫЕ ДАННЫЕ КОМПАНИИ ИЗ ЗАГРУЖЕННОГО ПРЕДЫДУЩЕГО ПАКЕТА»,
   это ПЕРИОДИКА ИЗ ЗАГРУЖЕННОГО ISO/СУОТ-пакета. Восстанови полную карточку из загруженных документов,
   а не создавай пустую новую компанию. Сохрани тип прошлого пакета: ISO, СУОТ или ISO+СУОТ. Старую дату
   выезда не выдавай за новую: если пользователь не назвал свежую дату, задай только этот вопрос перед
   формированием. Штат, объекты и поставщиков из прошлого пакета используй как базу и попроси подтвердить
...
def _run_archive_task(task_id):
    """Run one archive job using only its durable task metadata and upload file.

    The web process can restart during a deploy while an archive is being read.
    Keeping this runner outside the request handler makes the same task resumable
    after that restart instead of leaving the interface on "processing" forever.
    """
    ...
    # Upload names are generated by the server. Reject any old/corrupt metadata
    # that could otherwise point outside the persistent upload directory.
    if Path(upload_name).name != upload_name:
        upload_name = f'{task_id}.upload'
    archive_path = ARCHIVE_UPLOAD_DIR / upload_name
    filename = str(task.get('filename') or '')
    product = str(task.get('product') or 'all')
...
def _extract_spk_tools_from_copy_list(text: str) -> list:
    ...
    anchor = re.search(
        ...
        r'|перечень\s+средств\w*\s+измер)',
        value,
    )
    if not anchor:
        return []
