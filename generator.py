...
def generate_package(company_data: dict, api_key: str, product: str, progress_cb=None) -> dict:
    ...
    if company_data.get('workers'):
        for w in company_data['workers']:
            ...
            if pos and pos.strip().lower() not in existing_worker_positions:
...
№ | Дата | Наименование материала/изделия | Поставщик | № ТТН | Сопроводительные документы (сертификат/паспорт) | Результат контроля | Ответственный | Подпись


_____________________________________________
В настоящем журнале пронумеровано, прошнуровано и скреплено печатью _____ листов

Ответственный за входной контроль: {_fio(resp.get('process_resp'))} ({_pos(resp.get('process_resp'))})
...
№ | Дата | Наименование выполняемых работ | Объект | Объём выполненных работ | Результаты операционного контроля | Результаты приёмочного контроля | Ответственный | Подпись
...
ЖУРНАЛ УЧЁТА ТТК, ТНПА
(Приложение Б к Положению о системе производственного контроля)

Начат: _________________ Окончен: _________________

№ | Дата поступления | Обозначение ТНПА/ТТК | Наименование | Кол-во экземпляров | Контрольный/рабочий | Отметка об отмене/замене | Подпись


_____________________________________________
В настоящем журнале пронумеровано, прошнуровано и скреплено печатью _____ листов

Ответственный за учёт ТНПА и ТК: {_fio(resp.get('process_resp'))} ({_pos(resp.get('process_resp'))})
{dir_p(resp)} _____________ {_initials(_fio(resp.get('director')))}"""
...
ЖУРНАЛ УЧЁТА РЕКЛАМАЦИЙ ПО КАЧЕСТВУ СМР

Начат: _________________ Окончен: _________________

№ | Дата поступления | Заказчик / объект | Содержание претензии | Принятые меры | Срок устранения | Дата устранения | Ответственный | Подпись
...
ЖУРНАЛ УЧЁТА СРЕДСТВ ИЗМЕРЕНИЙ И ИСПЫТАТЕЛЬНОГО ОБОРУДОВАНИЯ
(Приложение В к Положению о системе производственного контроля)

Начат: _________________ Окончен: _________________

№ п/п | Наименование СИ | № (зав./инв.) | Технические характеристики (диапазон измерений, погрешность) | Дата постановки на производство | Сведения о поверке (калибровке), дата | Примечание (списание, ремонт)
...
(Приложение Д к Положению о системе производственного контроля)

Начат: _________________ Окончен: _________________

№ | Дата проведения аудита | Проверяемое направление | ФИО специалиста, проводившего внутренний аудит | Обнаруженное несоответствие | Ответственный за устранение | Мероприятия для устранения | Дата устранения | Отметка о выполнении
...
{dir_fio}, {dir_p(resp)}
Организация системы производственного контроля; общее руководство; принятие решений по качеству.

{spk_members}

Директор ______________ {dir_init}"""
    return text
...
def gen_spk_prikaz(num, name, company, dates, resp, itr, api_key):
    ...
    process_resp = resp.get('process_resp')
    fnpa_resp = resp.get('fnpa_resp')
    process_str = f"{_fio(process_resp)}, {_pos(process_resp)}" if process_resp else "ответственного"
    fnpa_str = f"{_fio(fnpa_resp)}, {_pos(fnpa_resp)}" if fnpa_resp and fnpa_resp != process_resp else process_str

    extras = {
        1: f"""Для осуществления постоянного контроля за соблюдением требований, установленных ТНПА, создать систему производственного контроля в соответствии с требованиями.
В системе производственного контроля задействовать: {process_str}{('; ' + fnpa_str) if fnpa_resp and fnpa_resp != process_resp else ''}.
{_pos(process_resp) if process_resp else 'Специалиста'} {_fio(process_resp) if process_resp else ''} назначить ответственным за: функционирование СПК; организацию проведения внутренних аудитов; входной контроль ПСД; ведение Журнала учёта рекламаций по качеству СМР и принятия мер по ним.
{('Назначить ' + fnpa_str + ' ответственным за: входной, операционный, приёмочный контроль; обеспечение и содержание в рабочем состоянии машин и механизмов; учёт, хранение, актуализацию, выдачу ТНПА, ТК; метрологическое обеспечение.') if fnpa_resp and fnpa_resp != process_resp else ''}""",
...
def gen_spk_polozhenie(company, dates, resp, itr, api_key):
    ...
    dir_fio = _fio(resp.get('director'))
    year = dates['year']
    specialists = ', '.join(_fio(p) + ' (' + _pos(p) + ')' for p in itr[:3])

    prompt = f"""Создай ПОЛОЖЕНИЕ О СИСТЕМЕ ПРОИЗВОДСТВЕННОГО КОНТРОЛЯ строго по этой структуре, максимально близко к формулировкам.

{full}
УТВЕРЖДАЮ {dir_p(resp)} {full} _____________ {dir_init} {dates['goals']} г.
ПОЛОЖЕНИЕ О СИСТЕМЕ ПРОИЗВОДСТВЕННОГО КОНТРОЛЯ {year}

1 ОБЩИЕ СВЕДЕНИЯ О СИСТЕМЕ ПРОИЗВОДСТВЕННОГО КОНТРОЛЯ
Настоящее положение распространяется на систему производственного контроля {full} и устанавливает функции, права, обязанности и ответственность специалистов, задействованных в системе производственного контроля, их взаимодействие.
Система производственного контроля предприятия создана с целью обеспечения возможности достижения требуемых показателей качества выполняемых работ, обеспечения единства и достоверности результатов контроля, получения объективной оценки качества выполняемых работ и обеспечения гарантии качества выполняемых работ.
Ответственным за функционирование системы производственного контроля является {dir_fio}, {dir_p(resp)}.
Организационная структура системы производственного контроля приведена в приложении А.
...
Основной задачей производственного контроля является предотвращение некачественного производства строительно-монтажных работ и достижение постоянного соответствия производимых работ требованиям ТНПА и проектной документации.
Основной функцией производственного контроля является обеспечение входного, операционного и приёмочного контроля выполняемых работ.
...
— вести отчётность по результатам измерений;
— своевременно информировать руководство о неудовлетворительных результатах контроля;
— проводить операционный и приёмочный контроль выполняемых работ с регистрацией в «Журнале производства работ» (СН 1.03.04-2020);
— осуществлять входной контроль качества закупаемой продукции согласно «Положению о входном контроле»;
— поддерживать в надлежащем состоянии испытательное оборудование и средства измерений, обеспечивать их своевременную поверку, калибровку и аттестацию;
— своевременно осуществлять учёт, актуализацию и тиражирование ТНПА и технологических карт.

2.3 Ответственность специалистов:
— за выполнение функций, предусмотренных настоящим Положением;
— за качество проводимых работ (достоверность, объективность, правильность);
— за приёмку некачественно выполненных работ в строительстве;
— за неудовлетворительную организацию производственного контроля.

2.4 Права специалистов:
— проводить входной, операционный и приёмочный контроль выполняемых работ;
— оформлять результаты испытаний в установленном порядке;
— оперативно информировать руководство о случаях несоответствия и вносить предложения об их приостановке.

Специалисты, задействованные в системе производственного контроля: {specialists}

3 ПОРЯДОК УПРАВЛЕНИЯ ДОКУМЕНТАЦИЕЙ И ЗАПИСЯМИ
3.1 Общие положения
На предприятии установлены и поддерживаются процедуры управления всеми документами, разработанными на предприятии или поступившими извне. Управлению подлежат: ТНПА; положения о структурных подразделениях, должностные инструкции; технологическая документация; проектно-сметная документация.

3.2 Управление ТНПА
Предприятие ежемесячно заключает договор с РУП «Стройтехнорм» на оперативное обеспечение актуальной информацией по ТНПА через информационно-поисковую систему «СтройДокумент-online». Все ТНПА, поступившие в бумажном виде, подлежат обязательному учёту и регистрации в журнале учёта ТТК, ТНПА (приложение Б). Отменённые ТНПА изымаются у пользователей, на них проставляется отметка «Отменён», утилизация производится по акту списания.

3.3 Положения о структурных подразделениях и должностные инструкции
Разрабатываются на каждого работника по должностям, включённым в штатное расписание, с учётом действующего законодательства и квалификационных справочников.

3.4 Технологическая документация
Предприятие приобретает необходимые типовые технологические карты у разработчиков. Технологические карты регистрируются в журнале учёта ТТК, ТНПА (приложение Б). Раз в 5 лет технологические карты пересматриваются.

3.5 Проектно-сметная документация
При заключении договора заказчик передаёт комплект проектно-сметной документации с отметкой «К производству работ» на каждом листе.

4 ПОРЯДОК УПРАВЛЕНИЯ СРЕДСТВАМИ КОНТРОЛЯ
4.1 Общие положения
Предприятие оснащено собственными средствами контроля в объёме требований ТНПА на проводимые виды контроля.

4.2 Регистрация и учёт СИ и ИО
Все средства измерений и испытательное оборудование зарегистрированы и учтены (приложение В). Каждая единица сопровождается индивидуальным паспортом с инвентарным номером.

4.3 Порядок предъявления СИ на поверку, калибровку, техническое обслуживание
СИ предъявляют на поверку, калибровку в соответствии с графиком, разработанным в соответствии с ТКП 8.003-2011. На все СИ наносятся этикетки (приложение Г).

Приложения: А — Организационная структура; Б — Журнал учёта ТТК, ТНПА; В — Журнал учёта средств измерений и испытательного оборудования; Г — Форма этикетки СИ; Д — Журнал регистрации внутренних аудитов.

Директор ______________ {dir_init} М.П.

Отвечай только текстом документа точно в этой структуре."""
    return vibe_call([{"role":"user","content":prompt}], api_key, max_tokens=4500)
...
def gen_spk_pasport(company, dates, resp, itr, api_key):
    ...
    prompt = f"""Создай ПАСПОРТ системы производственного контроля строго по этому формату:

{full}
УТВЕРЖДАЮ {dir_pos} {full} ______________ {dir_init} {dates['goals']}

ПАСПОРТ системы производственного контроля
{full}

1 Наименование и почтовый адрес организации, получившей свидетельство:
{full}
{company.get('address','')}

2 Фамилия, инициалы, должность, телефон руководителя организации:
{dir_pos} {dir_init} {dir_phone}

3 Наименования структурных подразделений и (или) должностных лиц, входящих в систему производственного контроля:
{spk_positions}

4 Фамилии, инициалы, должности руководителей структурных подразделений, а также должностных лиц, входящих в систему производственного контроля:
{itr_rows}

5 Дата и регистрационный номер свидетельства о технической компетентности, срок его действия:
Свидетельство о технической компетентности № __________ от __________ г.

6 Заявленные виды работ:
{chr(10).join(f"— {wt}" for wt in work_types)}
...
Входной контроль — проверка поступающих материалов и изделий по сопроводительным документам (накладные, сертификаты соответствия, паспорта качества) перед допуском к производству работ. Ответственный фиксирует результаты в Журнале входного контроля.
Операционный контроль — проверка соответствия выполняемых работ требованиям ТНПА и проектной документации на каждом этапе производства.
Приёмочный контроль — итоговая проверка качества выполненных работ с оформлением актов освидетельствования скрытых работ и исполнительной документации.

Директор ______________ {dir_init} М.П.

Отвечай только текстом документа точно в этом формате."""
    return vibe_call([{"role":"user","content":prompt}], api_key)
...
def gen_spk_spravka_ttk(company, dates, resp, work_types, api_key):
    ...
    TTK_DB = {
        ...
        'покраск': ('ТТК-191271613.7-2022', 'ТК на выполнение малярных работ', 'ГП «СтройМедиаПроект»'),
        'облицов': ('ТТК-100299864.152-2013', 'ТК на устройство облицовки плиткой', 'ГП «СтройМедиаПроект»'),
        'плиточн': ('ТТК-100299864.152-2013', 'ТК на устройство облицовки плиткой', 'ГП «СтройМедиаПроект»'),
        'кровел': ('ТТК-800017207.009-2023', 'ТК на устройство кровли из рулонных материалов', 'РУП «СТРОЙТЕХНОРМ»'),
        'электромонтаж': ('ТТК-100299864.155-2013', 'ТК на монтаж электромонтажных работ', 'ГП «СтройМедиаПроект»'),
        'окон': ('ТТК-100299864.152-2013', 'ТК на установку оконных и балконных дверных блоков из ПВХ профиля', 'ГП «СтройМедиаПроект»'),
        'полов': ('ТТК-100299864.101-2013', 'ТК на устройство полов', 'ГП «СтройМедиаПроект»'),
        'стяжк': ('ТТК-100299864.101-2013', 'ТК на устройство стяжки пола', 'ГП «СтройМедиаПроект»'),
        'плит': ('ТТК-100987457.203-2021', 'ТК на устройство сборного покрытия из плит тротуарных', 'РУП «СТРОЙТЕХНОРМ»'),
        'благоустройств': ('ТТК-100987457.203-2021', 'ТК на работы по благоустройству территорий', 'РУП «СТРОЙТЕХНОРМ»'),
        'бетон': ('ТТК-100299864.001-2013', 'ТК на бетонные работы', 'ГП «СтройМедиаПроект»'),
        'монтаж': ('ТТК-100299864.050-2013', 'ТК на монтажные работы', 'ГП «СтройМедиаПроект»'),
        'сварк': ('ТТК-100299864.060-2013', 'ТК на сварочные работы', 'ГП «СтройМедиаПроект»'),
        'сантехник': ('ТТК-100299864.200-2013', 'ТК на сантехнические работы', 'ГП «СтройМедиаПроект»'),
        'водопровод': ('ТТК-100299864.200-2013', 'ТК на монтаж систем водоснабжения', 'ГП «СтройМедиаПроект»'),
        'отоплен': ('ТТК-100299864.201-2013', 'ТК на монтаж системы отопления', 'ГП «СтройМедиаПроект»'),
    }
    ...
    for wt in work_types:
        wt_lower = wt.lower()
        ttk = None
        for key, val in TTK_DB.items():
            if key in wt_lower and val[0] not in used:
                ttk = val
                used.add(val[0])
                break
        if not ttk:
            ttk = ('ТТК-100299864.001-2013', f'ТК на {wt}', 'ГП «СтройМедиаПроект»')
        rows.append(f"{ttk[0]} | {ttk[1]} | {ttk[2]} | До {expire} г.")

    rows_text = chr(10).join(rows)

    text = f"""Перечень типовых технологических карт, имеющихся в наличии у {full}

№ п/п | Учетный № | Шифр | Наименование | Организация Разработчик | Срок действия
1     | 2         | 3    | 4            | 5                       | 6
...
def _select_si_for_work_types(work_types, has_welding, year):
    """Подбирает набор СИ исходя из реальных видов работ компании (не фиксированный список)"""
...
def gen_spk_spravka_si(company, dates, resp, api_key):
    ...
    year = dates['year']
    has_welding = company.get('has_welding', False)
    work_types = company.get('work_types', [])

    si_list = _select_si_for_work_types(work_types, has_welding, year)
    si_rows = '\n'.join(f"{i+1} | {s} | 1 | — | Свидетельство о поверке / счёт на поверку {year} г." for i, s in enumerate(si_list))

    prompt = f"""Создай СВЕДЕНИЯ О СРЕДСТВАХ ИЗМЕРЕНИЯ (СИ) для СПК.

{full}
Виды работ: {', '.join(work_types) if work_types else company.get('scope','')}

Сведения о средствах измерения (СИ)

Таблица (7 колонок): № п/п | Наименование и тип СИ | Технические характеристики (диапазон измерений, погрешность) | Количество, шт. | Заводской/учетный номер | Сведения о поверке или калибровке

Заполни реальными данными, набор СИ строго соответствует заявленным видам работ (не добавляй лишние приборы):
{si_rows}
...
def gen_spk_garantiynoe(num, name, company, dates, resp, api_key, bisp_org=None):
...
def gen_spk_grafik_poverki(company, dates, resp, api_key):
    ...
    base_si = _select_si_for_work_types(work_types, has_welding, year)

    si_rows = '\n'.join(f"{i+1} | {s} | 1 шт. | 1 раз в год | {year} г. | —" for i, s in enumerate(base_si))
...
Таблица: № | Наименование СИ и техническая характеристика | Кол-во | Периодичность поверки | Срок поверки | Отметка

{si_rows}

Ответственный: {_fio(resp.get('process_resp'))} ({_pos(resp.get('process_resp'))})
Отвечай только текстом документа в этом формате."""
    return vibe_call([{"role":"user","content":prompt}], api_key, max_tokens=2500)
...
def gen_spk_polozhenie_vhod_kontrol(company, dates, resp, api_key):
    ...
    dir_init = _initials(_fio(resp.get('director')))
    year = dates['year']
    prompt = f"""Создай ПОЛОЖЕНИЕ О ВХОДНОМ КОНТРОЛЕ для СПК строго в этом формате.

{full}
УТВЕРЖДАЮ {dir_p(resp)} {full} _____________ {dir_init} {dates['goals']} г.
ПОЛОЖЕНИЕ О ВХОДНОМ КОНТРОЛЕ {year}

1 ОБЩИЕ ПОЛОЖЕНИЯ
Входной контроль поступающей продукции (сырья, полуфабрикатов, комплектующих изделий) проводится с целью установления соответствия продукции требованиям технических нормативных правовых актов (далее ТНПА) и (или) договора (контракта) на поставку.
Задачами входного контроля являются:
— получение объективной и достаточной информации о качестве поставляемой продукции;
— предотвращение использования продукции, не соответствующей требованиям ТНПА и (или) договора (контракта) на поставку;
— обеспечение требуемого качества создаваемой продукции и выполняемых в строительстве работ.
Входной контроль проводится в соответствии с требованиями СТБ 1306-2002 «Строительство. Входной контроль продукции. Основные положения».
Испытания продукции при входном контроле проводятся по методикам, приведённым в технических нормативных правовых актах и (или) договоре (контракте) на поставку.
Входной контроль проводится до истечения гарантийного срока хранения продукции, но не позднее 10 дней после получения продукции из данного населённого пункта и не позднее 20 дней после получения из другого населённого пункта.

2 ПЕРЕЧЕНЬ ПРОДУКЦИИ, ПОДЛЕЖАЩЕЙ КОНТРОЛЮ
В {full} утверждён «Перечень продукции, подлежащей входному контролю» (Приложение А). Входному контролю подлежит вся поступающая продукция (сырьё, полуфабрикаты, комплектующие изделия, строительные материалы и изделия), используемая при производстве работ.

3 СХЕМЫ ВХОДНОГО КОНТРОЛЯ
Входной контроль в организации проводится по следующим схемам:
— Проведение испытаний по отдельным показателям качества, предусмотренным ТНПА и (или) договором (контрактом) на поставку (схема 5.1.2 по СТБ 1306-2002).
— Визуальный контроль, анализ приведённых в сопроводительной документации данных о качестве продукции (схема 5.1.3 по СТБ 1306-2002).

4 ПОРЯДОК ПРОВЕДЕНИЯ ВХОДНОГО КОНТРОЛЯ
Входной контроль продукции проводят путём анализа данных, приведённых в сопроводительной документации, удостоверяющей качество и комплектность продукции, визуального контроля поступившей продукции и проведения испытаний по всем или отдельным показателям качества, предусмотренным ТНПА и (или) договором (контрактом) на поставку.
В состав сопроводительной документации могут входить: документ о качестве изготовителя, сертификат соответствия на продукцию или техническое свидетельство, сертификат на систему качества.

Входной контроль по схеме 5.1.2 предусматривает выполнение следующих процедур:
— проверка наличия и комплектности сопроводительной документации;
— внешний осмотр поступившей продукции (сохранность упаковки, комплектность, наличие маркировки и т.д.);
— отбор проб для испытаний;
— проведение испытаний по показателям качества, указанным в ТНПА и (или) договоре (контракте) на поставку;
— регистрация результатов контроля и испытаний;
— обработка результатов входного контроля и принятие решения.
Результаты входного контроля считаются положительными, если поступившая продукция соответствует указанным в заказе требованиям ТНПА и (или) договора (контракта) на поставку.

Входной контроль по схеме 5.1.3 включает выполнение следующих процедур:
— проверка наличия и комплектности сопроводительной документации;
— внешний осмотр поступившей продукции (сохранность упаковки, комплектность, наличие маркировки и т.д.);
— проверка достаточности приведённых в сопроводительной документации данных о качестве и соответствии продукции требованиям ТНПА.

5 ДЕЙСТВИЯ ПРИ ВЫЯВЛЕНИИ НЕСООТВЕТСТВИЙ
При выявлении несоответствия продукции установленным требованиям продукция изолируется от качественной, на неё оформляется акт о несоответствии, поставщик уведомляется о выявленном несоответствии, принимается решение о возврате или утилизации продукции.

Ответственный за входной контроль: {_fio(resp.get('process_resp'))} ({_pos(resp.get('process_resp'))})

Директор ______________ {dir_init} М.П.

Отвечай только текстом документа точно в этой структуре и максимально близко к формулировкам."""
    return vibe_call([{"role":"user","content":prompt}], api_key, max_tokens=3500)
...
def gen_tech_trebovaniya(work_type, company, dates, resp, api_key):
    ...
    if tnpa:
        ...
        tnpa_text = f"""ТНПА: {sp_main} ({category}), {sp_control} (Контроль качества работ)

Ключевые параметры контроля:
{key_params}

Средства измерений (из Перечня СИ {full}): {si_names}"""
    else:
